...
// 四叉树存储的物体封装（包含位置、尺寸、原始物体引用）
    ...
    {
        public IntRect Bounds; // 物体的矩形边界（位置+尺寸）
        public GameObject Target; // 对应的Unity物体

        public QuadTreeObject(IntRect bounds, GameObject target)
        {
            Bounds = bounds;
            Target = target;
        }
...
// 四叉树核心类
    public class QuadTreeV1 : MonoBehaviour
    {
        public static QuadTreeV1 ins;

        [Header("四叉树配置")] [Tooltip("根节点覆盖的区域（整个地图范围）")]
        public Rect Bounds = Rect.zero;

        public IntRect RootRect;
        ...
        {
            ins = this;
            RootRect = new IntRect((int)(Bounds.x * QuadTreeConfig.BIGMULTIPLIER), (int)(Bounds.y *
                QuadTreeConfig.BIGMULTIPLIER), (int)(Bounds.width * QuadTreeConfig.BIGMULTIPLIER), (int)(Bounds.height *
                QuadTreeConfig.BIGMULTIPLIER));
        }
        ...
        // 对外接口：添加物体到四叉树
        public void AddObjectToQuadTree(IntRect bounds, GameObject target)
        {
            Debug.Log("add " + bounds.ToString());
        ...
        public void UpdateObjectPosition(GameObject target, IntRect newBounds)
        {
            // 1. 先移除旧位置的物体
        ...
        // 对外接口：查询指定区域内的所有物体
        public List<GameObject> GetObjectsInArea(IntRect area)
        {
            Debug.Log("click " + area.ToString());
            var result = new List<QuadTreeObject>();
            _rootNode.GetObjectsInArea(area, result);
            // 转换为GameObject列表返回
            return result.DistinctBy((q) => q.Bounds).ConvertAll(obj => obj.Target);
        }
        ...
        private void DrawNodeGizmos(QuadTreeNode node)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                Gizmos.DrawLine(vector2s[i] / QuadTreeConfig.BIGMULTIPLIER,
                    vector2s[(i + 1) % 4] / QuadTreeConfig.BIGMULTIPLIER);
            }
...
// 四叉树节点类（内部逻辑，不对外暴露）
    public class QuadTreeNode
    {
        ...
        public QuadTreeNode(IntRect rect, int currentDepth, int maxObjects, int maxDepth)
        {
            Rect = rect;
        ...
        private void Split()
        {
            ...
            // 创建四个子节点（左上、右上、左下、右下）
            NW = new QuadTreeNode(new IntRect(x, y + halfHeight, halfWidth, halfHeight), _currentDepth + 1, _maxObjects,
                _maxDepth);
            NE = new QuadTreeNode(new IntRect(x + halfWidth, y + halfHeight, halfWidth, halfHeight), _currentDepth + 1,
                _maxObjects, _maxDepth);
            SW = new QuadTreeNode(new IntRect(x, y, halfWidth, halfHeight), _currentDepth + 1, _maxObjects, _maxDepth);
            SE = new QuadTreeNode(new IntRect(x + halfWidth, y, halfWidth, halfHeight), _currentDepth + 1, _maxObjects,
                _maxDepth);
        }
        ...
        // 查询指定区域内的所有物体
        public void GetObjectsInArea(IntRect area, List<QuadTreeObject> result)
        {
        ...
        private void TryMergeNodes()
        {
            ...
            if (NW.IsSplit || NE.IsSplit || SW.IsSplit || SE.IsSplit)
            {
                //还有细分 无法合并
                return;
            }
            ...
            uniqueChildObjects = uniqueChildObjects.DistinctBy((q) => q.Bounds);
